  ...
  for (int i = 0; i < measurements.size(); i++) {
    for (int j = 0; j < measurements[i].size(); j++) {
      ...
      for (int k = 0; k < landmark.points.size(); k++) {
        ...
        //        if (error > 100) {
        //          std::cout << "Skipping point " << k << " of landmark " <<
        //          landmark.id << " in frame " << i << ". Residual: "
        //              << error << std::endl;
        //          continue;
        //        }
        ...
        problem.AddResidualBlock(
            ...
            landmarks.at(observation.id).pose.data(), camera_poses[i].data(),
...
void BundleAdjuster::SetParametersConstant() {
  ...
  std::vector<int> constant_landmark_params = {(int)POSE::Rx, (int)POSE::Ry,
                                               (int)POSE::Rz};
  ceres::SubsetParameterization *constant_landmark_params_mask =
      new ceres::SubsetParameterization((int)POSE::N_PARAMS,
                                        constant_landmark_params);
  ...
  std::vector<int> constant_vehicle_params = {(int)POSE::Z, (int)POSE::Rx,
                                              (int)POSE::Ry};
  ceres::SubsetParameterization *constant_vehicle_params_mask =
      new ceres::SubsetParameterization((int)POSE::N_PARAMS,
                                        constant_vehicle_params);
