...
std::vector<cv::Point> LandmarkFinder::FindPoints(cv::Mat &i_oGrayImage) {
  ...
  FindClusters(Pixels, ClusteredPixels, m_fMaxRadiusForPixelCluster,
               m_nMinPixelForCluster, m_nMaxPixelForCluster);
  ...
  for (pAllClustersIt = ClusteredPixels.begin();
       pAllClustersIt != ClusteredPixels.end(); pAllClustersIt++) {
    fPixelsForPoint = 0;
