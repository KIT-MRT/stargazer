...
//
//ASSERT_EQ(expected, actual);   EXPECT_FLOAT_EQ(expected, actual);   the two float values are almost equal (4 ULPs)
//ASSERT_EQ(expected, actual);  EXPECT_DOUBLE_EQ(expected, actual);  the two double values are almost equal (4 ULPs)
//ASSERT_EQ(val1, val2, abs_error);  EXPECT_NEAR(val1, val2, abs_error);  the difference between val1 and val2 doesn't exceed the given absolute error
//
...
TEST(ConfigHandler, Read) {
  ...
  ASSERT_EQ(1, camera_intrinsics[(int) INTRINSICS::alpha]);
  ASSERT_EQ(1, camera_intrinsics[(int) INTRINSICS::beta]);
  ASSERT_EQ(1.570796327, camera_intrinsics[(int) INTRINSICS::theta]);
...
TEST(ConfigHandler, Write) {
  ...
  ASSERT_TRUE(writeConfig(testfile,
                          camera_intrinsics,
                          landmarks));
